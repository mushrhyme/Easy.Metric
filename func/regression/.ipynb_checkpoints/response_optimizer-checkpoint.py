...
def create_response_optimizer(df, reg_model, X):
    ...
    fig = make_subplots(
        rows=n_rows, 
        cols=n_cols,
        # subplot_titles=[f"<b>{name}</b>" for name in predictors],
    ...
    # 각 변수별 시각화 생성
    for idx, feature in enumerate(predictors, 1):
        row = (idx - 1) // n_cols + 1
        col = (idx - 1) % n_cols + 1
    ...
    fig.update_layout(
        title={
            'text': f'<b>{st.session_state.equation}</b><br><sup>현재 예측값: {prediction:.2f}</sup>',
            'y': 0.90,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top'
...
def create_inverse_optimizer(df, reg_model, target_value, constraints=None):
    """
    목표값에 가장 근접한 예측변수 조합을 찾는 함수
    
    Parameters:
    df: DataFrame - 원본 데이터
    reg_model: statsmodels 회귀모델
    target_value: float - 목표 예측값
    constraints: dict - 예측변수별 제약조건 (optional)
...
def create_response_optimizer(df, reg_model, X):
    ...
    fig.update_layout(
        title={
            'text': f'<b>현재 예측값: {prediction:.2f}</b>',
            'y': 0.90,
            'x': 0.5,
            'xanchor': 'center',
            'yanchor': 'top'
...
def create_response_optimizer(df, reg_model, X):
    ...
    fig = make_subplots(
        ...
        cols=n_cols,
        subplot_titles=sub_titles,
        vertical_spacing=0.1,  # 서브플롯 간 간격 줄임
        horizontal_spacing=0.15
    )
    ...
    # 여백에 방정식을 표시하기 위한 추가 공간 확보
    equation_height = 13  # 방정식을 위한 추가 높이

    # 방정식 줄바꿈 적용
    wrapped_equation = wrap_equation(st.session_state.equation)
    num_lines = wrapped_equation.count("<br>") + 1
    equation_height = max(100, num_lines * 30)
    
    # 그래프 영역을 위로 올리고 아래 여백 확보
    for i in range(1, n_rows + 1):
        for j in range(1, n_cols + 1):
            fig.update_yaxes(
                row=i, 
                col=j,
                domain=[0.15, 0.85]  # y축 영역을 위로 조정 (0-1 사이의 값)
            )
    
    # 레이아웃 업데이트
    total_height = 350 * n_rows + equation_height  # 전체 높이
    ...
    fig.update_layout(
        ...
        margin=dict(
            ...
            l=50,    # 왼쪽 여백
            r=50,    # 오른쪽 여백
            b=equation_height + 40  # 하단 여백 크게 증가
        )
    )
    ...
    fig.add_annotation(
        ...
        x=0.5,
        y=0,  # 페이퍼의 하단
        yshift=-(equation_height/2),  # 아래로 이동
        showarrow=False,
        font=dict(size=12),
        align='center',
        bgcolor=colors['background'],  # 배경색 추가
        bordercolor=colors['background'],  # 테두리 색상
