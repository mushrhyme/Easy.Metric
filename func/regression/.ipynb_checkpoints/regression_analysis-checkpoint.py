...
def calculate_sequential_ss(df, target, terms):
    """
    순차적으로 SS를 계산하는 함수
    """
    ss_values = []
    ...
    for i in range(len(terms)):
        current_formula = f"{target} ~ " + " + ".join(terms[:i+1])
        ...
        if previous_model is None:
            ss = np.sum((current_model.fittedvalues - df[target].mean())**2)
        else:
...
def calculate_adjusted_ss(df, target, terms):
    """
...
def run_anova(df):
    ...
    if len(categorical_vars) > 0:
        # 범주형 변수들을 Category 타입으로 변환
        for var in categorical_vars:
            df[var] = df[var].astype('category')
    
    # 수식 생성
    interactions = [i.split(":")[0]+"_sq" if i.split(":")[0]==i.split(":")[1] else i for i in st.session_state.interactions]
    terms = st.session_state.predictor + interactions
    ss_values, df_values, final_model = calculate_sequential_ss(df, st.session_state.target, terms)
    adj_ss = calculate_adjusted_ss(df, st.session_state.target, terms)
    
    # ANOVA 테이블 생성
    results_df = pd.DataFrame(index=['회귀'] + terms + ['오차', '총계'])
    
    # 총 제곱합과 자유도 계산
    SST = np.sum((df["C3"] - df["C3"].mean())**2)
    SSE = np.sum((df["C3"] - final_model.fittedvalues)**2)
    SSR = SST - SSE
    ...
    # Sequential SS 할당 (표시용)
    results_df.loc['회귀', ['Seq SS', 'DF']] = [SSR, DF_reg]
    for term, ss, df_term in zip(terms, ss_values, df_values):
        results_df.loc[term, 'Seq SS'] = ss
        results_df.loc[term, 'DF'] = df_term
    
    # Adjusted SS 할당 (회귀는 Sequential SS 사용)
    results_df.loc['회귀', 'Adj SS'] = SSR  # 회귀는 Sequential SS를 Adj SS로 사용
    for term in terms:
        if term in adj_ss:  # Intercept 제외
    ...
    results_df.index = list(map(lambda x: x.split("_sq")[0] +":" + x.split("_sq")[0] if "_sq" in x else x, results_df.index))
