...
def calculate_sequential_ss(df, target, terms, name_mapping):
    ...
    def process_term(term):
        if ':' in term:
            ...
            if var1 == var2:
                # 제곱항인 경우 (ex: C1:C1)
                safe_var = name_mapping[var1]
                safe_df[f"{safe_var}_sq"] = df[var1] ** 2
                return f"{safe_var}_sq"
            else:
                # 일반 교호작용인 경우
                safe_var1 = name_mapping[var1]
                safe_var2 = name_mapping[var2]
                interaction_name = f"{safe_var1}_{safe_var2}"
                safe_df[interaction_name] = df[var1] * df[var2]
                return interaction_name
        else:
            # 일반 변수인 경우
            return name_mapping[term]

    # 각 항에 대해 안전한 변수명으로 변환
    safe_terms = [process_term(term) for term in terms]
...
def calculate_adjusted_ss(df, target, terms, name_mapping):
    ...
    # 제곱항과 교호작용에 대한 특별 처리 추가
    result = {}
    for k, v in type3['sum_sq'].to_dict().items():
        if k.endswith('_sq'):
            # 제곱항인 경우
            original_var = reverse_mapping[k[:-3]]
            result[f"{original_var}:{original_var}"] = v
        elif '_' in k:
            # 교호작용인 경우
            var1, var2 = k.split('_')
            original_var1 = reverse_mapping[var1]
            original_var2 = reverse_mapping[var2]
            result[f"{original_var1}:{original_var2}"] = v
        else:
            # 일반 변수인 경우
            result[reverse_mapping.get(k, k)] = v
