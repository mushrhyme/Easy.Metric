...
def run_gage_rnr():
    df = pd.DataFrame({
        "Part": st.session_state.df[st.session_state.part],
        "Operator": st.session_state.df[st.session_state.operator],
        "Measurement": st.session_state.df[st.session_state.measurement]
    })
    # 이원 분산 분석 수행
    ...
    # p-value 재계산
    anova_results.loc["C(Part)", "PR(>F)"] = 1 - stats.f.cdf(anova_results.loc["C(Part)", "F"], 
                                                            anova_results.loc["C(Part)", "df"], 
                                                            anova_results.loc["C(Part):C(Operator)", "df"])
    
    anova_results.loc["C(Operator)", "PR(>F)"] = 1 - stats.f.cdf(anova_results.loc["C(Operator)", "F"], 
...
def draw_meas_by_part():
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
            x=st.session_state.df['Part'], 
            y=st.session_state.df['Measurement'], 
            mode='markers',
            name='반응',
    ...
    # 평균을 이은 선 그리기
    part_means = st.session_state.df.groupby('Part')['Measurement'].mean()
    fig.add_trace(
        go.Scatter(
            x=part_means.index, 
            y=part_means.values, 
...
def draw_R_ctrl_chart():
    fig = go.Figure()
    r_values = st.session_state.df.groupby(['Operator', 'Part'])['Measurement'].agg(lambda x: x.max() - x.min())
...
def draw_meas_by_operator():
    fig = go.Figure()
    fig.add_trace(
        go.Box(
            x=st.session_state.df['Operator'],
            y=st.session_state.df['Measurement'],
            showlegend=False
        )
    )
    
    operator_means = st.session_state.df.groupby('Operator')['Measurement'].mean()
    fig.add_trace(
        go.Scatter(
...
def draw_xbar_ctrl_chart():
    fig = go.Figure()
    xbar_values = st.session_state.df.groupby(['Operator', 'Part'])['Measurement'].mean()
...
def draw_interaction():
    ...
    interaction_means = st.session_state.df.groupby(['Part', 'Operator'])['Measurement'].mean().unstack()
...
def viz_gage_rnr():
    ...
    with col4:
        draw_meas_by_operator()

    col5, col6 = st.columns(2)
    
    with col5:
        draw_xbar_ctrl_chart()
